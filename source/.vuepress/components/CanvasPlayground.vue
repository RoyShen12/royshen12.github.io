<template>
  <div class="cv-wrapper">
    ...
    <div class="cs_row" style="margin: 1rem 0 .3rem 0;">
    ...
    <div class="cs_row">
      <!-- <textarea class="cs_text" v-model="jsContent" style="font-size: 13px;max-height: 20rem;height: 20rem;">
      </textarea> -->
...
export default {
  ...
  data() {
    return {
      jsContent: this.js.replace(/\\n/g, '\n').replace(/\\\\r/g, '>'),
      envContent: this.env ? this.env.replace(/\\n/g, '\n').replace(/\\\\r/g, '>') : '',
      ctx: null,
      cmOptions: {
        mode: 'javascript',
        theme: 'vscode-dark',
  ...
  mounted() {
    ...
    const [height, width] = isMobile ? [Math.round(innerWidth * .6), innerWidth - (innerWidth / 16 * 2.2)] : [300, 500]
